...
<section id="content">
	...
	?>
			<article class="item">
				...
				<?php if((isset($optionsdb['cwp_roby_b_featureimg']) && $optionsdb['cwp_roby_b_featureimg'] == 'Show') || !isset($optionsdb['cwp_roby_b_featureimg'])) { ?>
					<div class="image">
						<?php
				...
				<div class="readmore"><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"><?php _e('More','cwp_roby'); ?></a></div>

			</article><!--index article end-->
	<?php endwhile;
	/* Use PageNavi*/ else : ?>
	<?php	_e('404 Not Found','cwp_roby') ?>;
	<?php endif; ?>
			<?php
				$p = get_previous_posts_link( __( 'Previous page','cwp_roby' ) );
				$n = get_next_posts_link( __( 'Next page','cwp_roby' ) );
