<?php
	class cwp_robySanitizer{
		...
		public function sanitize_background($input,$default = array("bgcolor"=>"","bgimage"=>"","bgrepeat"=>"no-repeat","bgposition"=>"center center","bgattachment"=>"scroll")){
				...
				$att = array_keys($att);
				$input['bgcolor'] = apply_filters("cwp_roby_sanitize_color",$input['bgcolor'],$default['bgcolor']);
				$input['bgimage'] = apply_filters("cwp_roby_sanitize_url",$input['bgimage'],$default['bgimage']);
		...
		public function sanitize_typography($input,$default = array("color"=>"","size"=>12,"style"=>"normal","font"=>"arial")){
				...
				$input['color'] = apply_filters("cwp_roby_sanitize_color",$input['color'],$default['color']);
		...
		public function sanitize_enum($input,$name, $default = ''){
				...
				if(in_array($input,array_keys($options)))
					return apply_filters("cwp_roby_sanitize_textarea",$input);
				return $default;
		}
